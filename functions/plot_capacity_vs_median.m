function plot_capacity_vs_median(Capacity_partial, p50, cm_main, lo, hi) 
...
% ---- Data
X = (Capacity_partial - mean(Capacity_partial)) ./ mean(Capacity_partial) * 100;  % [%]
Y = p50;
...
% ---- Figure
figure('Color','w','Name','Capacity deviation vs median HI-d'); hold on
...
% ---- Labels & style
xlabel('Relative Capacity Deviation [%]');
ylabel('HI-d Median [%]');
xlim(xL); ylim(yL);
...
colormap(cm_main);
caxis([lo hi]);
cb = colorbar; ylabel(cb, 'HI-d Median [%]');
